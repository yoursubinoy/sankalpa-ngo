...
function GalleryImageItem({ image, index }: { image: string; index: number }) {
  ...
  return (
    <div
      ref={ref}
      className={`relative w-full sm:w-1/2 md:w-1/3 lg:w-1/4 h-48 sm:h-48 md:h-64 lg:h-72 hover-lift animate-on-scroll ${
        isVisible ? "visible animate-scale-in" : ""
      }`}
...
export default function GalleryPage({
  ...
  const slug = parseInt(resolvedParams.slug, 10);
  const events = [
    {
      ...
      <main className="flex-grow mt-10">
        <section className="py-16 px-4">
          <div className="container mx-auto">
            <h1
              ref={titleRef}
              className={`text-4xl font-bold mb-8 text-center animate-on-scroll ${
